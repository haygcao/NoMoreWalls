...
class SiblingTracksSheet extends HookConsumerWidget {
  ...
  // 修改这里，使用 title 属性替代 name
  final title = ServiceUtils.getTitle(
    activeTrack?.title ?? "",
    // 修改这里，不再使用 artists 属性
    artists: activeTrack?.artistName != null ? [activeTrack!.artistName] : [],
    onlyCleanArtist: true,
  ).trim();
  // 修改这里，使用 artistName 属性替代 artists?.asString()
  final defaultSearchTerm =
      "$title - ${activeTrack?.artistName ?? ""}";
  ...
  final searchRequest = useMemoized(() async {
    ...
    // 使用 SourceManager 进行搜索
    final results = await SourceManager.instance.search(searchTerm);
  ...
  final itemBuilder = useCallback(
    (SourceInfo sourceInfo) {
      final icon = SourceManager.sourceIcons[sourceInfo.runtimeType];
      return ListTile(
