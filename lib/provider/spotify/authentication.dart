...
class AuthenticationNotifier extends AsyncNotifier<AuthenticationTableData?> {
  ...
  build() async {
    ...
    // 替换已弃用的 listenSelf 方法
    ref.listen(
      spotifyAuthenticationProvider, 
      (prevData, newData) async {
        if (newData.value == null) return;

        if (newData.value!.isExpired) {
        ...
        refreshTimer?.cancel();
        refreshTimer = Timer(
          newData.value!.expiration.difference(DateTime.now()),
          () => refreshCredentials(),
        );
      },
    );
...
// 重命名为 spotifyAuthenticationProvider 以明确其用途
final spotifyAuthenticationProvider =
